...
class PathPlan(Node):
    ...
    def __init__(self):
        ...
        self.plan_new_path_sub = self.create_subscription(Point, "/heist/plan_new_path", self.plan_new_path_cb, 10)
    ...
    def plan_new_path_cb(self, msg: Point):
        self.goal_x = msg.x
        self.goal_y = msg.y
        if self.debug:
            self.get_logger().info(f"Received new path planning request to ({self.goal_x}, {self.goal_y})")
